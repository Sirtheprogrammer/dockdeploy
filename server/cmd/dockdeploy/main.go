...
func run() error {
	...
	if *versionFlag || *vFlag {
		fmt.Printf("dockdeploy version %s\n", api.Version)
		return nil
	}

	if *portFlag > 0 {
		os.Setenv("PORT", strconv.Itoa(*portFlag))
	}
	if *dbFlag != "" {
	...
	fmt.Printf("\n" +
		"  ┌─────────────────────────────────────────────────────────────┐\n" +
		"  │                     dockdeploy control plane                │\n" +
		"  │  Version:  %-48s │\n" +
		"  │  Database: %-48s │\n" +
		"  │  URL:      %-48s │\n" +
		"  └─────────────────────────────────────────────────────────────┘\n\n",
		api.Version, string(cfg.DatabaseDriver)+": "+cfg.DatabaseURL, cfg.AppURL)

	if cfg.AutoGeneratedSecrets {
		log.Info("auto-generated persistent secrets for local zero-dependency operation", "key_file", cfg.SecretsPath)
	} else if cfg.SecretsPath != "" {
		log.Info("loaded persistent secrets from key file", "key_file", cfg.SecretsPath)
	}
